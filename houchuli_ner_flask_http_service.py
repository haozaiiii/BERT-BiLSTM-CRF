...
from afterprocess.after_process import remove_not_cloud_in_train_data
from bert_base.recommendation.recommendcore import get_suggest_word
...
if os.name == 'nt':
    ...
    train_word_feq_path = config['WIN']['train_word_feq_path']
    ...
    train_word_feq_path = config['LINUX']['train_word_feq_path']
...
# 加载train_word_feq词典
train_word_feq_set = set()
with open(train_word_feq_path,'r',encoding='utf-8') as rf:
    lines = rf.read().replace('\r','').split('\n')
    for line in lines:
        train_word_feq_set.add(line.split('\t')[0])
    ...
    with graph.as_default():
        ...
        try:
            ...
            for sentence in sentences:
                ...
                # err_index_end_list = remove_not_cloud_in_train_data(train_word_feq_set, err_index_end_list)
...
def get_suggest_word_http_request(sen_dict):
    ...
    response = requests.post(ngram_suggest_url, data=json.dumps(sen_dict), headers=headers,timeout=2)
    if response.status_code == 200:
        return json.loads(response.content)
    else:
        return sen_dict
def ppl_predicts_service(sen_dict):
    get_ngram_score_url = config['LINUX']['get_ngram_score_url']
    get_ppl_score_url = config['LINUX']['get_ppl_score_url']
    ngram_top_n = config['LINUX']['ngram_top_n']
    ppl_word_n = config['LINUX']['ppl_word_n']

    sen_dict = get_suggest_word(sen_dict, get_ngram_score_url, get_ppl_score_url, ngram_top_n, ppl_word_n)
