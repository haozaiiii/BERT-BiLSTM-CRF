        ...
        try:
            ...
            for sentence in sentences:
                ...
                if len(sentence) < 2:
                    ...
                    # sentence, pred_label_result[0] = recovery_data_format(pre_sen,sentence,pred_label_result[0])
                    print('原文 : ', str(pre_sen).lower())
                    print('token : ', sentence)
                    print('tag : ', pred_label_result[0])
                    sys.stdout.flush()
                ...
                print('111',sen_dict)
                sen_dict = recovery_data_format_p(pre_pre_sen,sen_dict)
        ...
        try:
            ...
            for sentence in sentences:
                ...
                if len(sentence) < 2:
                    ...
                    pred_label_result = convert_id_to_label(pred_ids_result, id2label)
                    # sentence,pred_label_result[0] = recovery_data_format(pre_sen,sentence,pred_label_result[0])
                    print('原文 : ', str(pre_pre_sen).lower())
                    print('token : ', sentence)
                    print('tag : ', pred_label_result[0])
                    sys.stdout.flush()
                    err_index_end_list = find_index_int_token_tag(str(pre_sen).lower(), sentence, pred_label_result[0])
...
def recovery_data_format_p(pre_pre_sent,sen_dict):
    tmp_str = ''
    for k in pre_pre_sent:
        tmp_str += k
    pre_pre_sent = re.sub('[0-9]', '*', pre_pre_sent)
    pre_pre_sent = re.sub('×', '*', pre_pre_sent)


    """恢复数据格式，将剔除的空字符恢复"""
    error_index_true_content_list = list(sen_dict['sentence'])
    for item in sen_dict['items']:
        if 'suggest_word' not in item:
            item['suggest_word'] =['']

        replace_index = 0
        replace_word = item['suggest_word'][0]
        for i in range(item['start'],item['end']):
            if len(replace_word) > 0:
                print(str(i), str(replace_word[replace_index]))
                error_index_true_content_list[i] = replace_word[replace_index]
                replace_index += 1
            else:
                error_index_true_content_list[i] = sen_dict['sentence'][i]
    error_index_true_content = ''.join(error_index_true_content_list)
    if len(pre_pre_sent) == len(sen_dict['sentence']):
        sen_dict['sentence'] = tmp_str
        return sen_dict
    else:
        correct_sent = ''
        remove_index_list = []
        pre_index = 0
        after_index = 0
        after_len = len(sen_dict['sentence'])
        while after_index < after_len:
            if pre_pre_sent[pre_index] != sen_dict['sentence'][after_index]:
                remove_index_list.append(pre_index)
                correct_sent += pre_pre_sent[pre_index]
                pre_index += 1
            else:
                correct_sent += error_index_true_content[after_index]
                pre_index +=1
                after_index +=1
        for item in sen_dict['items']:
            start_index = item['start']
            end_index = item['end']
            for k in remove_index_list:
                if k < start_index:
                    start_index += 1
                if k < end_index:
                    end_index += 1

            item['start'] = start_index
            item['end'] = end_index
            item['token'] = pre_pre_sent[start_index:end_index]
            if len(item['suggest_word'][0]) > 0:
                item['suggest_word'][0] = correct_sent[start_index:end_index]
...
def recovery_data_format(text, pred_ids,pred_label):
    """恢复数据格式，将剔除的空字符恢复"""
    char_list = list(text)
    if len(char_list) == len(pred_ids):
        # 原字符中不包含空字符
        return pred_ids, pred_label
    ...
    for char in char_list:
        if index == len(pred_ids):
            new_pred_ids.append(char)
            new_pred_label.append("O")
            continue

        ids = pred_ids[index]
        label = pred_label[index]
        if ids == char:
            new_pred_ids.append(ids)
            new_pred_label.append(label)
            index += 1
            continue
        ...
        if index == 0:
            ...
            last_label = pred_label[index -1]
            label = last_label.replace("B-", "I-")
            new_pred_label.append(label)
    return new_pred_ids, new_pred_label
...
if __name__ == "__main__":
    ...
    if not server_passwd2.server_passwd2():
        print('key error ...')
        sys.exit(-1)
