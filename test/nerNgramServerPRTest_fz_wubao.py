...
def predict(path):
    ...
    for line in contentList:
        ...
        for one_err_dict in return_dict['result'][0]['items']:
            ...
            elif len(one_err_dict['suggest_word']) >0 and (one_err_dict['token'] in correct_sentence or correct_sentence in one_err_dict['token']):
                correct_count += 1
            ...
            #           str(correct_sentence[int(one_err_dict['start']): int(one_err_dict['end'])]))
                ...
                print('原文!=建议 ： ', str(one_err_dict['token']), correct_sentence)
